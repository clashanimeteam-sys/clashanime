-- Production: channel violations / copyright strikes
...
drop policy if exists "Users can view own channel violations" on public.channel_violations;
create policy "Users can view own channel violations"
  on public.channel_violations for select
  using (auth.uid() = user_id);

drop policy if exists "Staff can view all channel violations" on public.channel_violations;
create policy "Staff can view all channel violations"
  on public.channel_violations for select
  using (public.is_staff());

drop policy if exists "Staff can insert channel violations" on public.channel_violations;
create policy "Staff can insert channel violations"
  on public.channel_violations for insert
  with check (public.is_staff());

drop policy if exists "Staff can update channel violations" on public.channel_violations;
...
select count(*) as channel_violations_table_ready from public.channel_violations;
