...
select
  ...
  (select count(*) from public.verification_requests where status = 'pending') as pending_verification_requests,
  (select count(*) from public.referral_clicks) as referral_clicks_count;
