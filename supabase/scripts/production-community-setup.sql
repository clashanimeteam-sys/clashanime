-- Production: Community engagement (images, reactions, comments, post reports)
...
drop policy if exists "Anyone can read community post reactions" on public.community_post_reactions;
create policy "Anyone can read community post reactions"
  on public.community_post_reactions for select
  using (true);

drop policy if exists "Users can react to community posts" on public.community_post_reactions;
create policy "Users can react to community posts"
  on public.community_post_reactions for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update own community post reactions" on public.community_post_reactions;
create policy "Users can update own community post reactions"
  on public.community_post_reactions for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "Users can remove own community post reactions" on public.community_post_reactions;
...
drop policy if exists "Users can post community comments" on public.community_post_comments;
...
drop policy if exists "Staff can delete community post comments" on public.community_post_comments;
...
drop policy if exists "Staff can delete community posts" on public.community_posts;
...
select
  ...
  (select count(*) from storage.buckets where id = 'community-images') as community_images_bucket;
