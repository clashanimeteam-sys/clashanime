...
-- WhatsApp contact preference for payout KYC.

alter table public.payout_kyc_submissions
  add column if not exists whatsapp_opt_in boolean not null default false,
  add column if not exists whatsapp_phone text;

drop function if exists public.submit_payout_kyc(text, text, text, text, text, text, text);
...
grant execute on function public.submit_payout_kyc(text, text, text, text, text, text, text, boolean, text) to authenticated;
