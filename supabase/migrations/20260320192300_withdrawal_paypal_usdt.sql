-- Add PayPal and Crypto USDT payout methods alongside bank transfer.
...
alter table public.withdrawals
  ...
  check (payment_method in ('bank_transfer', 'paypal', 'crypto_usdt'));

drop function if exists public.request_clash_coin_withdrawal(integer, text, text, text, boolean);
...
grant execute on function public.request_clash_coin_withdrawal(integer, text, jsonb, boolean) to authenticated;
